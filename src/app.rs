...
impl eframe::App for AudioFilterApp {
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        if *(&self.filter_freq_res.is_none()) {
            ...
            svf.update_coefficients(1000.0, 0.707);
            let mut impulse: Vec<f32> = (0..sample_rate).map(|_| 0.0).collect();
            impulse[0] = 1.0;

            let mut impulse_response = impulse.iter().map(|sample| svf.render(*sample)).collect::<Vec<f32>>();

            let mut real_planner = RealFftPlanner::<f32>::new();
            let r2c = real_planner.plan_fft_forward(sample_rate);
            let mut spectrum = r2c.make_output_vec();
            r2c.process(&mut impulse_response[..], &mut spectrum).expect("failed to process FFT");
            ...
            self.filter_freq_res = Some(spectrum.iter().map(|f| (f.re.abs() + std::f32::EPSILON).log10() * 20.0).collect());
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            if let Some(filter_freq_res) = &self.filter_freq_res {
                let sin: PlotPoints = filter_freq_res.iter().enumerate().map(|(x, y)| {
                    [x as f64, *y as f64]
                }).collect();
                let line = Line::new(sin);
                Plot::new("frequencies").view_aspect(2.0).show(ui, |plot_ui| plot_ui.line(line));
            }
        });
    }
}
